...
import { SERVER_BASE_URL } from '../utils/strings.utils';
dotEnv.config();
...
export interface IEmailsRepo {
  ...
  createWelcomeEmail(payload: confirmationEmailRequests): Promise<unknown>;
...
export const EmailsRepo: IEmailsRepo = {
  async createConfirmationEmail(payload: confirmationEmailRequests): Promise<unknown> {
    try {
      ...
      const { firstname, lastname, email, public_key, token, confirm_id } = payload;

      const confirmation_link = `${SERVER_BASE_URL}/users/v1/confirm/${confirm_id}/${token}`;
      ...
      const mailOptions: MailOptions = {
        ...
        to: email,
        subject: 'Verify your Ductape.io account',
        // @ts-ignore
        template: 'confirm',
        context: { firstname, lastname, token, year, confirmation_link },
        /**attachments: [
  ...
  async createWelcomeEmail(payload: confirmationEmailRequests): Promise<unknown> {
    try {
      const viewPath = path.resolve(__dirname, '../templates/views/');
      const partialsPath = path.resolve(__dirname, '../templates/partials');
